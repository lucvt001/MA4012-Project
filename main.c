    ...
    switch (current_state)
    {
      ...
      case SEARCHING:
        ...
        if (!timer_started)
        {
          clearTimer(T1);
          timer_started = true;
          rotate(20, search_bias);
        }        

        // Stop for abit before reversing the turn
        int turn_duration = 750; // Time in ms to turn in one direction before switching
        ...
        else if (time1[T1] >= turn_duration + pause_duration && time1[T1] < turn_duration * 3 + pause_duration)
        {
          rotate(20, -search_bias);  // Reverse spin direction
        }
        ...
        else if (time1[T1] >= turn_duration * 3 + pause_duration)
        {
          timer_started = false;  // Reset timer for next use
          rotate(0, 0);
